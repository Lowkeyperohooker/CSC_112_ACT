...
void print_error(struct Error *token) {
    printf("\nToken name: %s", token->name);
    printf("\nToken start: %s", token->start);
    printf("\nToken line: %d", token->line);
}

void print_symbol(struct Symbol *token) {
    printf("\nToken type: %s", token->name);
    printf("\nToken name: %d", token->is_initiallized);
    printf("\nToken line: %d", token->memory_address);
}
...
void print_node(struct ParseTree *node) {
    printf("\nNode type: %s", get_node(node-> type));
    print_token(node -> token);
}

void print_errors(struct Error *token) {
    while(token) {
        struct Error *next = token->next;
...
void print_symbols(struct Symbol *token) {
    while(token) {
        struct Symbol *next = token->next;
...
void compile(char *source_code, char *output_name) {
    ...
    printf("\n\nParse Tree Successfully shown.");
    printf("\n\nSymbols.");
    print_symbols(head_symbol);
    printf("\n\nErrors.");
